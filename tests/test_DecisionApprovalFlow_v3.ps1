...
$projectRoot = Split-Path $controlPlaneDir
...
$unauthResult = Invoke-TestCommand -ScriptPath "$controlPlaneDir\governance-enforcement.ps1" -Description "Testing governance enforcement for unauthorized execution" -Parameters @{
    TaskContractPath = "nonexistent.yaml"
    ProjectTruthPath = $projectTruthPath
...
$approveResult = Invoke-TestCommand -ScriptPath "$controlPlaneDir\State-Machine.ps1" -Description "Approving test task" -Parameters @{
    TaskId = "test-auth-task"
...
$execResult = Invoke-TestCommand -ScriptPath "$controlPlaneDir\governance-enforcement.ps1" -Description "Executing authorized task" -Parameters @{
    TaskContractPath = $taskContractPath
    ProjectTruthPath = $projectTruthPath
    ProjectStatePath = $projectStatePath
}
...
$validateResult = Invoke-TestCommand -ScriptPath "$controlPlaneDir\validation-engine.ps1" -Description "Validating task execution" -Parameters @{
    TaskId = "test-auth-task"
    ProjectStatePath = $projectStatePath
}
...
$evidenceCheck = Invoke-TestCommand -ScriptPath "$controlPlaneDir\State-Machine.ps1" -Description "Checking evidence for task" -Parameters @{
    TaskId = "test-auth-task"
    ProjectStatePath = $projectStatePath
    Action = "get-evidence"
}
